...
def get_current_svn_cfg():
    "Gets the svn info for each checkout"
    logger(get_func_name() + "()", "debug")
    global releaseCfg
    xxx = []
...
def update_svn_checkouts():
    "Does an svn update, switch or checkout for each target dir"
    logger(get_func_name() + "()", "debug")
    global releaseCfg

def clear_old_deploy_files():
    "Removes the webapp and clears tomcat work cache and tmp dir"
    logger(get_func_name() + "()", "debug")
    global releaseCfg

def copy_new_deploy_files():
    "Copies in the new war and property files"
    logger(get_func_name() + "()", "debug")
    global releaseCfg

def show_config():
    "Prints the app and release configs and exits"
    logger(get_func_name() + "()", "debug")
    global releaseCfg
    global myCfg
